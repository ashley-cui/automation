...
stop_listener(){
    ...
    # Clean up setup state files and GitHub runner credentials
    # Runner will be re-registered when instance is re-enabled
    $SSH ec2-user@$pub_dns rm -f \
        "./.setup.done" \
        "./.setup.started" \
        "/var/tmp/shutdown.sh" \
        "/Users/${name}-worker/actions-runner/.credentials" \
        "/Users/${name}-worker/actions-runner/.runner"
}
...
for _dhentry in "${_dhstate[@]}"; do
    ...
    if ! $SSH ec2-user@$pub_dns test -r .setup.done; then
        ...
        if ! $SSH ec2-user@$pub_dns test -r .setup.started; then
            ...
            if ! $SCP $SETUP_SCRIPT $SPOOL_SCRIPT $SHDWN_SCRIPT $CLEANUP_HOOK_SCRIPT ec2-user@$pub_dns:/var/tmp/; then
            ...
            # Clear sensitive token from environment now that it's been passed to setup.sh
