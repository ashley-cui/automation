...
CLEANUP_HOOK="/var/tmp/cleanup_hook.sh"
...
if [[ ! -r "$RUNNER_CONFIG_FILE" ]]; then
    ...
    msg "Configuring job hooks and environment"
    sudo -u $PWUSER bash -c "cat > $RUNNER_DIR/.env <<EOF
ACTIONS_RUNNER_HOOK_JOB_COMPLETED=$CLEANUP_HOOK
GOPATH=/Users/$PWUSER/ci/.go
GOCACHE=/Users/$PWUSER/ci/.go/cache
EOF
"
fi
